...
- auf Basis der *unabhängigen* Variabken `time` und `size` ($x_1, x_2$):
...
- Je nach dem, welche Variablen zur Modellierung verwendet werden, ergeben sich i.d.R. verschiedene Prognosen: `lm(y ~ 1); lm(y ~ x1); lm(y ~ x1 + x2); lm (y ~ x1 * x2)`
